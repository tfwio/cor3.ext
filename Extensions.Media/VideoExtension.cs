...
namespace System.Cor3.Mvc
{
	...
	{
		public static bool HasVideoFile(this Controller controller, out string cat, out string file)
		{
			cat = file = null;
		...
		public static bool HasImageFile(this Controller controller, out string cat, out string file)
		{
			cat = file = null;
			if (!controller.CheckParam("cat", out cat)) return false;
			if (!controller.CheckParam("file", out file)) return false;
		...
		public static void GetVideoFile(this Controller controller, string file)
		{
		...
		internal static void ServeVideo(this Controller controller, string filename)
		{
			controller.ServeVideo(filename,4096*32,300);
		}
		internal static void ContentTypeFromVideoExtension(this Controller controller, FileInfo fi)
		{
			switch (fi.Extension)
			{
		...
		internal static void ServeVideo(this Controller controller, string filename, int buffersize, int sleepInterval)
		{
	...
	//			controller.Response.AppendHeader("Content-Type", "video/x-flv");
		...
		internal static void ServeImage(this Controller controller, string filename)
		{
			controller.ServeImage(filename,1024,900);
		}
		internal static void ServeImage(this Controller controller, string filename, int buffersize, int sleepInterval)
		{
			FileInfo fi = new FileInfo(filename);
			switch (fi.Extension)
			{
