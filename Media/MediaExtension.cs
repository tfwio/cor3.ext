...
namespace System.Cor3.Mvc.Media
{
    ...
    <embed
		...
		static public int SortFileList(this Controller controller, FileInfo f1, FileInfo f2)
		{
			return f1.Name.CompareTo(f2.Name);
		}

		static public void checkForMovieDir(this Controller controller)
		{
			...
			else
			{
				...
				foreach (FileInfo fi in listA)
				{
					list.Add(fi.Name.HtmlA(
						"javascript:messageIt('"+Uri.EscapeDataString(@"{root}?cat=vid&file=".Glob()+fi.FullName.Replace("\\","/").Replace(@"'",@"\'")+@"')"),
						null,
						null,
						null,
						null
		...
		static public void checkForMovies(this Controller controller)
		{
